		...
		if cmdstats {
			...
			*/
			...
			if len(frags) > 3 {
				...
				if rt, err = extractVal(frags[4]); err != nil {
					continue
				}
				if maxRt, err = extractVal(frags[5]); err != nil {
					continue
				}
				if requestLen, err = extractVal(frags[6]); err != nil {
					continue
				}
				if responseLen, err = extractVal(frags[7]); err != nil {
					continue
				}
			}
